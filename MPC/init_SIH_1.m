...
if(use_beta)
    ...
    model.dyn_fun = @(x,u) dynamics_SIH_1(x,u,param,model.beta_fun);
else
...
objective = struct;
if(use_beta)
    objective.stage_cost = @(x,u) q*(N*k_icu*x(3) - C_d)^2 + r*(max(control_list)-u)^2;
    objective.term_cost = @(x) q_T*(N*k_icu*x(3) - C_d)^2;
    % Constraints
    objective.constraints.u_max = max(control_list);
    objective.constraints.u_min = min(control_list);
else
    objective.stage_cost = @(x,u) q*(N*k_icu*x(3) - C_d)^2 + r*(n_control-1-u)^2;
    objective.term_cost = @(x) q_T*(N*k_icu*x(3) - C_d)^2;
    % Constraints
...
opt.horizon  = 200;  % [days] time horizon
